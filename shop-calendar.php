...
class ShopCalendar {
	...
	public function check_my_options() {
		...
		// バージョン1.1のオプション初期化
			...
			if ($this->options['long_holiday1']['month'] != 0) {
				$setting['long_holiday1']['year'] =  date_i18n('Y');
			}
			if ($this->options['long_holiday2']['month'] != 0) {
				$setting['long_holiday2']['year'] =  date_i18n('Y');
			}
			if ($this->options['long_holiday3']['month'] != 0) {
				$setting['long_holiday3']['year'] =  date_i18n('Y');
			}
			update_option("shop_calendar", $setting);
		}
	}
	...
	public function page_init() {
		...
		add_settings_field('no_holiday', '臨時営業日', array($this, 'no_holiday_callback'), 'shop_calendar', 'shop_calendar_section_id');
	}
...
//			var_dump($this->options); echo '<br/>';
	...
	private function check_holiday($i, $counter, $year, $month, $day) {
		...
		*/
		...
		if ($year == $now_year && $no_holiday_month && $no_holiday) {
			if ($day == $no_holiday) {
				return false;
			}
		}
